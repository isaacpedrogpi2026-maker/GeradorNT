...
def has_effective_value(value):
    """Check if a value has effective content (not empty, NaN, or placeholder)."""
    if pd.isna(value):
        return False
    if isinstance(value, str):
        text = value.strip()
        if text == '' or text.lower() in ['n/a', 'na', 'nan', 'none', '[informação não disponível]']:
            return False
        return True
    # For non-string values (numbers, etc.), consider them as having effective value
    # unless they are NaN (already handled above)
    return True
...
def prepare_obras_for_display(df_segreg):
    if df_segreg is None or df_segreg.empty:
        return pd.DataFrame()
...
def extract_dados_gerais(df_segreg, df_volumes, df_abastecimento, df_populacao, df_tv_tspe=None):
    ...
    # 7 - Índice de Atendimento de Água (from Segreg por município)
    # Usar o df_segreg original que foi passado como parâmetro (já filtrado por município)
    indices_agua_col = find_column(df_segreg, ['Índice de Atendimento de Água', 'Indice de Atendimento de Agua', 'Indice Atendimento Agua'])
    if indices_agua_col and indices_agua_col in df_segreg.columns:
        # Pega o primeiro valor não vazio
        for value in df_segreg[indices_agua_col].dropna():
            text = str(value).strip()
            if text not in ['', 'nan', 'None']:
                dados_gerais['indice_atendimento_agua'] = value
                break
        else:
            dados_gerais['indice_atendimento_agua'] = '[Informação não disponível]'
    else:
        dados_gerais['indice_atendimento_agua'] = '[Informação não disponível]'
    
    # 8 - Índice de Atendimento de Esgoto (from Segreg por município)
    indices_esgoto_col = find_column(df_segreg, ['Índice de Atendimento de Esgoto', 'Indice de Atendimento de Esgoto', 'Indice Atendimento Esgoto'])
    if indices_esgoto_col and indices_esgoto_col in df_segreg.columns:
        for value in df_segreg[indices_esgoto_col].dropna():
            text = str(value).strip()
            if text not in ['', 'nan', 'None']:
                dados_gerais['indice_atendimento_esgoto'] = value
                break
        else:
            dados_gerais['indice_atendimento_esgoto'] = '[Informação não disponível]'
    else:
        dados_gerais['indice_atendimento_esgoto'] = '[Informação não disponível]'
...
def extract_dados_obras(df_segreg):
    ...
    # Se o DataFrame estiver vazio, retorna um dicionário vazio
    if df.empty:
        return {}
...
def build_document_model(selected_obras_by_status, dados_gerais, dados_obras, selected_municipio):
    ...
    # Índices de Atendimento - sempre incluir, mesmo se não disponível
    indice_atendimento_agua = normalize_missing_value(
        dados_gerais.get('indice_atendimento_agua', '[Informação não disponível]')
    )
    indice_atendimento_esgoto = normalize_missing_value(
        dados_gerais.get('indice_atendimento_esgoto', '[Informação não disponível]')
    )

    # Construir a lista de dados gerais SEMPRE incluindo os índices
    general = [
        ...
        ('Índice de Atendimento de Água', indice_atendimento_agua),
        ('Índice de Atendimento de Esgoto', indice_atendimento_esgoto),
    ...
    # Filtrar apenas linhas que têm valor efetivo, mas manter os índices mesmo se forem "[Informação não disponível]"
    # Para os índices, queremos que apareçam sempre, então não os filtramos
    general_filtered = []
    for label, value in general:
        # Sempre incluir os índices de atendimento
        if label in ['Índice de Atendimento de Água', 'Índice de Atendimento de Esgoto']:
            general_filtered.append((label, value))
        # Para os outros campos, filtrar apenas se tiverem valor efetivo
        elif has_effective_value(value):
            general_filtered.append((label, value))
    
    general = general_filtered
...
def display_dados_gerais(dados_gerais):
    ...
    with col1:
        ...
        st.markdown("**Índice de Atendimento de Água:**")
        st.write(normalize_missing_value(dados_gerais.get('indice_atendimento_agua', '[Informação não disponível]')))
    
    with col2:
        st.markdown("**Índice de Atendimento de Esgoto:**")
        st.write(normalize_missing_value(dados_gerais.get('indice_atendimento_esgoto', '[Informação não disponível]')))
...
# ========== FERRAMENTA: GERADOR DE NOTAS TÉCNICAS ==========
def ferramenta_gerador_notas_tecnicas():
    ...
    if raw_excel_file is not None:
        try:
            ...
            segreg_sheet = find_sheet_name(dataframes, ['Segreg por município', 'Segreg por municipio', 'Investimentos', 'Investimento'], 
                                          required_columns=['Status', 'Total do Investimento R$ por município', 'Total do Investimento'])
            volumes_sheet = find_sheet_name(dataframes, ['Volumes por município', 'Volumes por municipio', 'Volumes'], 
                                           required_columns=['Vazão [L/s]', 'Vazao [L/s]', 'Vazão', 'Vazao'])
            abastecimento_sheet = find_sheet_name(dataframes, ['Abastecimento'], 
                                                  required_columns=['Sistema de Abastecimento', 'Calendário Médio Atual', 'População Total Residente SINISA 2024'])
            populacao_sheet = find_sheet_name(dataframes, ['População por município', 'Populacao por município', 'População por municipio', 'Populacao por municipio'], 
                                              required_columns=['TV + TSPE (Econ)', 'População Beneficiada'])
            tv_tspe_sheet = find_sheet_name(dataframes, ['TV+TSPE', 'TV + TSPE', 'TV+TSPE (Econ)', 'TV + TSPE (Econ)'], 
                                            required_columns=['TV + TSPE (Econ)'])
            investimentos_sheet = find_sheet_name(dataframes, ['Investimentos', 'Investimento'], 
                                                  required_columns=['Município Principal', 'Municipio Principal'])
            ...
            total_investment = sum(float(v.get('valor', 0) or 0) for v in dados_obras.values())
            ...
            # Mesmo sem investimentos, exibe a seção DADOS DAS OBRAS e mantém o botão de download
            st.header("📊 DADOS DAS OBRAS")
            ...
            # Construir modelo mesmo sem investimentos
...
def main():
    ferramenta_gerador_notas_tecnicas()
...
#             # Mesmo sem investimentos, exibe a seção DADOS DAS OBRAS
#             st.header("📊 DADOS DAS OBRAS")
#             if total_investment <= 0:
#                 st.markdown(f"""
#                 <div style="display:flex; justify-content:center; align-items:center; min-height:100px; margin-bottom:20px;">
